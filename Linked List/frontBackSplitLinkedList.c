/* Write a C function frontBackSplitLinkedList() that splits the singly linked list into two sublists – one for the front half, and one for the back half. 
...
{
    /* add your code here */
    int i=0,m=0,j=ll->size;
    int frontLastI=0,backStartI=0;
    ...
    backStartI= frontLastI +1;
    frontPtr = ll->head;
    backPtr = findNode (ll, backStartI);
    
    while (i<backStartI)
    {
        insertNode(resultFrontList,i,frontPtr->item);
    ...
    while (i<j)
    {
        insertNode(resultBackList,m,backPtr->item);

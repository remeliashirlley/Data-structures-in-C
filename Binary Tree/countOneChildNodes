/* Write a C function countOneChildNodes() that accepts a pointer to the root node of a binary tree and returns the number of nodes that have exactly one child node. */
...

{
    ...
    result = countOneChildNodes(node->left) + countOneChildNodes(node->right);
    return result;
}

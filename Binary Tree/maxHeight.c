/* Write a C function maxHeight() that accepts a pointer to the root node of a binary tree
and return the number of links along the longest path from the root node down to the farthest leaf node.
...

{
    ...
    leftH = maxHeight(node->left) + 1;
    rightH = maxHeight(node->right) + 1;
